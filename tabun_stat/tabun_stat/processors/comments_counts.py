...
class CommentsCountsProcessor(BaseProcessor):
    ...
    def start(
        ...
        self._fp_sum = open(
            os.path.join(self.stat.destination, "comments_counts_sum.csv"), "w", encoding="utf-8"
        )
        self._fp_perc = open(
            os.path.join(self.stat.destination, "comments_counts_perc.csv"), "w", encoding="utf-8"
        )
    ...
    def process_comment(self, comment: types.Comment) -> None:
        ...
        try:
            ...
            self.stat.log(
                0, f"WARNING: comments_counts: comment {comment.id} for unknown post {comment.post_id}"
            )
            return
    ...
    def _flush_stat(self) -> None:
        ...
        all_count = (
            self._last_comment_id - self._first_comment_id + 1
        )  # Число комментов вместе с неизвестными
        if self._last_comment_id == 0:
            all_count = 0
        if all_count < all_exist_count:
            self.stat.log(
                0, "WARNING: inconsistent comments count: {} < {}!".format(all_count, all_exist_count)
            )
