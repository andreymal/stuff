...
class Sqlite3DataSource(BaseDataSource):
    ...
    def _fetch_blog_info(self) -> None:
        ...
        self._blog_status_by_id = {x["id"]: x["status"] for x in blogs_status}
        self._blog_status_by_slug = {x["slug"]: x["status"] for x in blogs_status}
    ...
    def iter_posts(
        ...
        while last_id < stat.last_id:
            posts = self.fetchall_dict(
                f"select * from posts where id >= ?{where} order by id limit 1000",
                (last_id + 1,) + where_args,
            )
            if not posts:
                break

            comments_dict: dict[int, list[types.Comment]] | None = None
            if with_comments:
    ...
    def get_post_comments(self, post_id: int) -> list[types.Comment]:
        items = self.fetchall_dict("select * from comments where post_id = ?", (post_id,))
    ...
    def iter_comments(
        ...
        while True:
            comments = self.fetchall_dict(
                f"select * from comments where id >= ?{where} order by id limit 10000",
                (last_id + 1,) + where_args,
            )
            if not comments:
                break
